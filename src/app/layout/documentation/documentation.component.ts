...
it('chat() should invoke Chat with message',
  inject([DocumentationComponent], (component: DocumentationComponent) => {
    ...
    spyOn(connectionMockMgr.mock, 'invoke').and.returnValue(Promise.resolve([]));

    component.message = 'My message';
    component.chat();

    expect(connectionMockMgr.mock.invoke)
...
it('ngInit() should listen for broadcast event',
  inject([DocumentationComponent], (component: DocumentationComponent) => {
    ...
    spyOn(connectionMockMgr.mock, 'listen').and.callThrough();

    component.ngOnInit();

    expect(connectionMockMgr.mock.listen)
      .toHaveBeenCalledWith(jasmine.objectContaining({ event: 'OnMessageSent' }));

    expect(connectionMock.invokeListeners['OnMessageSent'].observers.length)
      .toBe(1);
